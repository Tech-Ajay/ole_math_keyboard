...
import 'package:ole_math_keyboard/src/foundation/math2tex.dart';
...
import 'package:ole_math_keyboard/src/widgets/math_keyboard.dart';
...
class _MathFieldState extends State<MathField> with TickerProviderStateMixin {
  ...
  KeyEventResult? _handleCharacter(
    ...
    for (final config in configs) {
      ...
      if (config.keyboardCharacters
        ...
        if (basicConfig.args != null) {
          _controller.addFunction(basicConfig.value, basicConfig.args!, basicConfig.label);
        } else {
    ...
    //   return KeyEventResult.handled;
    // }
    //  if (lowerCaseCharacter == 'o') {
    //   _controller.addLeaf('{\oint}');
    //   return KeyEventResult.handled;
    // }
...
class _FieldPreview extends StatelessWidget {
  ...
  Color _getDisabledCursorColor(ThemeData themeData) {
    ...
    const darkEnabled = Colors.white;
    const darkDisabled = Colors.white;
    const lightEnabled = Colors.white;
    const lightDisabled = Colors.white;
...
class MathFieldEditingController extends ChangeNotifier {
  ...
  Future<void> addLeaf(String tex) async {
    
    if(tex.length>1)
    {
      SharedPreferences pref = await SharedPreferences.getInstance();
      recentlist =  pref.getStringList("recent") ?? [];
      String x = "$tex , $tex , [] " ;
      print("😎💖😎😎😎😎😎😎😎😎😎$x");
      recentlist.add(x);
      recentlist = recentlist.toSet().toList();
      pref.setStringList("recent", recentlist);
  ...
  Future<void> addFunction(String tex, List<TeXArg> args, String label) async {
    currentNode.removeCursor();
    final func = TeXFunction(tex, currentNode, args);
    SharedPreferences pref = await SharedPreferences.getInstance();
      recentlist =  pref.getStringList("recent") ?? [];
      recentlist = recentlist.toSet().toList();
    String x = "$tex , $label , $args ";
    // print("😎💖😎😎😎😎😎😎😎😎😎$x");
    recentlist.add(x);
          recentlist = recentlist.toSet().toList();
